# Azure deployment script for PTSA Tracker (Fixed version)
...
Write-Log "Starting Azure deployment..." "Green"
...
# 3) ACR (build/push locally to avoid ACR Tasks)
$acrLoginServer = $null
$acrUsername = $null
$acrPassword = $null
try {
...
# 4) App Service Plan (Free to avoid quota issues)
try {
    az appservice plan show --name "asp-$AppName" --resource-group $ResourceGroupName 1>$null 2>&1
    if ($LASTEXITCODE -ne 0) {
        az appservice plan create --name "asp-$AppName" --resource-group $ResourceGroupName --sku F1 --is-linux 1>$null
    }
    Write-Log "App Service Plan ready." "Green"
} catch {
    Write-Log "App Service Plan error: $_" "Yellow"
}

# 5) PostgreSQL Flexible Server (use valid Burstable SKU)
$dbAdminPassword = "PtsaTracker2024!"
try {
    az postgres flexible-server show --name $DBServerName --resource-group $ResourceGroupName 1>$null 2>&1
    if ($LASTEXITCODE -ne 0) {
        az postgres flexible-server create `
            --name $DBServerName `
            --resource-group $ResourceGroupName `
            --location $Location `
            --admin-user ptsaadmin `
            ...
            --storage-size 32 `
            --yes 1>$null
        az postgres flexible-server db create --resource-group $ResourceGroupName --server-name $DBServerName --database-name $DBName 1>$null
    }
    $dbConnectionString = "postgresql://ptsaadmin:$dbAdminPassword@$DBServerName.postgres.database.azure.com:5432/$DBName"
    Write-Log "PostgreSQL ready." "Green"
} catch {
    $dbConnectionString = "sqlite:///instance/ptsa.db"
    Write-Log "PostgreSQL step failed: $_ - falling back to SQLite." "Yellow"
}
...
try {
    ...
    # Note MAIL_DEFAULT_SENDER is single-quoted to allow < and >
    ...
    Write-Log "1. Verify your Azure subscription has quota for App Service Plans" "White"
    Write-Log "2. Consider using a different region if you are having resource issues" "White"
    Write-Log "3. Try deploying manually through Azure Portal" "White"
    
    Write-Log "Alternative Deployment Options:" "Yellow"
    Write-Log "1. Use local deployment with the command: python deploy_db.py" "White"
    Write-Log "2. Consider using Azure Web Apps for Linux (without containers)" "White"
    Write-Log "3. Try using Azure SQL instead of PostgreSQL" "White"
    exit 1
}

Write-Log "Deployment script completed successfully!" "Green"
