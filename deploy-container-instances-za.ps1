...
if ($acrLoginServer) {
    ...
    try {
        ...
        Write-Log "⚠️ Custom image build failed, using fallback" "Yellow"
    }
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($imageToUse.Contains("nginx")) {
            Write-Log "ℹ️  This is a placeholder container (nginx)" "Yellow"
        ...
        Write-Log "" "White"
        Write-Log "💰 Cost: ~\$5-10/month (much cheaper than App Service)" "Green"
